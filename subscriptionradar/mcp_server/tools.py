...
def handle_savings_report(*, db_path: Path, days: int = 30, limit: int = 10) -> str:
    """Analyze recent subscription-related articles for cost-saving insights."""
    cutoff = datetime.now(tz=UTC) - timedelta(days=days)
    conn = duckdb.connect(str(db_path), read_only=True)
    try:
        rows = conn.execute(
    ...
    price_keywords = {"price increase", "price hike", "가격 인상", "요금 인상", "구독료"}
    alerts: list[str] = []
    for title, source, entities_json, collected_at in rows:
        ...
        for entity_name, matched in entities.items():
            if not isinstance(matched, list):
                continue
            hits = [kw for kw in matched if isinstance(kw, str) and kw.lower() in price_keywords]
            if hits:
                alerts.append(f"- [{entity_name}] {title} ({source}, {collected_at})")

    if not alerts:
        return "No price change alerts found in the recent period."

    header = f"Savings report ({len(alerts)} alert(s) in last {days} days):"
    return "\n".join([header, *alerts[:limit]])
